...
export const routes: Routes = [
    ...
    {path: 'admin', canActivate: [authGuard], loadComponent: () => import('./layouts/admin-layout/admin-layout').then(c => c.AdminLayout),
        children: [
            ...
            {path: 'teacher', canActivate: [roleGuard], data: { roles: ['admin']}, loadComponent: () => import('./feature/teacher/teacher').then(c => c.Teacher)},
            {path: 'subject', canActivate: [roleGuard], data: { roles: ['admin']}, loadComponent: () => import('./feature/subject/subject').then(c => c.Subject)},
            {path: 'account', canActivate: [roleGuard], data: { roles: ['admin']}, loadComponent: () => import('./feature/account/account').then(c => c.Account)}
        ]
    },
];
